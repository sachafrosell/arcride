...
export default function App(props) {
  ...
  React.useEffect(() => {
    ...
    if (props.verifyUpload && testUpload && mounted) {
      ...
      const storageRef = storage.ref(`/Drivers/${userID}/commendation`);
      const task = storageRef.put(file);
      task.on(firebase.storage.TaskEvent.STATE_CHANGED, {
        'next': (snapshot) => {
          let progress = Math.round((snapshot.bytesTransferred / snapshot.totalBytes) * 100);
          props.uploadPercentage("commendation", progress);
        },
        'complete': () => {
  ...
  React.useEffect(() => {
    //let mounted = true;
    if (url !== "" && stopLoop) {
      props.setFinishUpload("commendation")
      props.documentURL("commendation", url)
      setStopLoop(false)
    }
    //return () => mounted = false;
  }, [url, stopLoop, props])
  ...
  return (
    ...
    <div>
      ...
      <form >
        ...
        Upload Letter of Commendation from previous employer or local chief
        </div>
